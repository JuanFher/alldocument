<template>
<div class="hidden w-[300px] shrink-0 overflow-y-auto overflow-x-hidden px-2.5 pt-2 lg:block xl:w-[320px] 2xl:w-[360px]">
    <!--Is empty clipboard-->
    ...
    <TitlePreview v-if="!isSingleFile && !isEmpty" class="mb-6" icon="check-square" :title="$t('selected_multiple')" :subtitle="this.clipboard.length + ' ' + $tc('items', this.clipboard.length)" />
    ...
    <div v-if="isSingleFile && !isEmpty">
        ...
        <!--Filesize-->
        <ListInfoItem v-if="singleFile.data.attributes.filesize" :title="$t('size')" :content="singleFile.data.attributes.filesize" />

        <!--Cat-->
        <ListInfoItem title="Categoría" :content="singleFile.data.attributes.tipo_nombre" />

        <!--Fields-->
        <div v-if="singleFile.data.attributes.meta_data.length > 0">
            <ListInfoItem v-for="(campo, index) in singleFile.data.attributes.meta_data" v-bind:key="index" :title="campo.key" :content="campo.value" />
        </div>
        <!--Created At-->
...
: $getCurrentLocationName()
        ...
        <!--Shared-->
        <ListInfoItem v-if="$checkPermission('master') && singleFile.data.relationships.shared" :title="$t('shared')">
